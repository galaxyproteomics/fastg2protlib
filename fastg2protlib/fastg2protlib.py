...
from fastg2protlib.diagnostics import generate_diagnostic_plots
...
def _translate_dna(seq, min_length=150):
    def frame_translation(sequence):
        frame_sequences = []
        # translate for each frame
        for frame_start in range(3):
            frame_seq = sequence[frame_start:]
            rem = len(frame_seq) % 3
            if rem == 1:
                frame_seq += "NN"
            if rem == 2:
            ...
            frame_sequences.append(f"{t_seq}")
    ...
    translated_sequences = frame_translation(seq)
    rev_compl = Seq(seq).reverse_complement()
    translated_sequences.extend(frame_translation(f"{rev_compl}"))

    return_list = []
    for a_sequence in translated_sequences:
        return_list.extend(list(filter(filter_protein, a_sequence.split("*"))))
    return return_list
...
def write_peptide_fasta(db, fasta_name="peptide.fasta"):
    # (1, '1,1,600')
    logger.info("Executing select query for peptide writing.")
...
def peptides_for_fastg(
    ...
    db_name="tst.db",
    peptide_file_name="peptide.fasta",
    create_plots=False
):
    # region
    """
    ...
    for walk in walks:
        ...
        if len(obj['proteins']) > 0:
            protein_walk_objs.append(obj)
        else:
            no_protein_count += 1
    logger.info("End walks")
    logger.info(f"There were {no_protein_count} walks with no protein sequences.")
    ...
    # Write peptide fasta
    write_peptide_fasta(db, fasta_name=peptide_file_name)

    # diagnostics
    if create_plots:
        logger.info("Generating diagnostic plots")
        generate_diagnostic_plots(db)
